import React, {Component} from 'react';
import './App.css';
import L from 'leaflet';
import {Map,TileLayer,Marker} from 'react-leaflet';
import Weather from './Components/Weather';

var myIcon = L.icon({
 iconUrl: 'data:image/png;base64,',
 iconSize: [25, 41],
 iconAnchor: [12.5, 41],
 popupAnchor: [0,-41]
});


const apiKey = 'cebad3e6604a3d079faf82653bcf2b6d';
var URI= `https://api.openweathermap.org/data/2.5/weather?appid=${apiKey}&q=`; 


export default class App extends Component {

  constructor(){
    super();
  
  this.state = {
    visibleMap: false,       
    visibleWeather: false,       
    visibleGuide: false,
    lat: '55.8651505',
    lng: '-4.2576299',
    myCity:'Glasgow',
    countryCode:'Uk',
    zoom: 13    
  };

  this.handleChange = this.handleChange.bind(this); 
}

getWeather(myCity,countryCode){
   return this.http.get(`${this.URI}${myCity},${countryCode}`);
   this.getWeather(myCity,countryCode)
   .subscribe(
     res => {
       this.weather = res
     }, 
     err => console.log(err)
   )

}

submitLocation(myCity, countryCode){
  if (myCity.value && countryCode.value){

  this.getWeather(myCity.value, countryCode.value); 

  myCity.value= '';
  countryCode.value='';
  } else {
    alert ('Please insert a value');
  }
  myCity.focus();

  return false;
}



handleChange(e) {
  const { name, value } = e.target;
  this.setState({
    [name]: value
  });
}

MapChange() {
          
  this.setState({
    visibleMap: true,       
    visibleWeather: false,       
    visibleGuide: false
  });
}

WeatherChange() {
          
  this.setState({
    visibleMap: false,       
    visibleWeather: true,       
    visibleGuide: false
  });
}

GuideChange() {
          
  this.setState({
    visibleMap: false,       
    visibleWeather: false,       
    visibleGuide: true
  });
}

 

  render () {
    var position = [this.state.lat, this.state.lng];
    
  return (

<div>
  <div>
    <nav className="nav-extended light-green darken-1">
                <div className="nav-wrapper">
                  <a href="#" className="brand-logo">
                    <img width="35%" src="https://www.laguiadeviaje.com/wp-content/uploads/2014/10/GuiaViaje.jpg"  alt="logo"></img>
                  </a>
                  <ul  className="right hide-on-med-and-down">
                <li className="tab "><a onClick={()=>{this.MapChange()}} href="#"> City Map</a></li>                    
                <li className="tab "><a onClick={()=>{this.WeatherChange()}} href="#"> Weather</a></li>
                <li className="tab "><a onClick={()=>{this.GuideChange()}} href="#"> Guide</a></li>
                    
                  </ul>
                </div>      
                            
    </nav>

    <div className="nav-extended light-green darken-1">
    <p className= "pt-2 text-white text-center">Introduce your city</p>  
    <div className="nav-extended ">
      <p className="text-white">Latitude</p>
      <input className= "white" name="lat" onChange={this.handleChange} value={this.state.lat} type="text" placeholder="Latitude" autoFocus/>
      <p className="pt-2 text-white">Longitude</p>
      <input className= "white" name="lng" onChange={this.handleChange} value={this.state.lng} type="text" placeholder="Longitude" autoFocus/>
      <p className="pt-2 text-white">City</p>
      <input className= "white" name="myCity" onChange={this.handleChange} value={this.state.myCity} type="text" placeholder="City" autoFocus/>
      
    </div>  
    </div>
    
</div>

{ this.state.visibleMap == true ? 

<Map className="map" center={position} zoom={this.state.zoom}>
        <TileLayer
          attribution='&amp;copy <a href="http://osm.org/copyright">OpenStreetMap</a> contributors'
          url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
        />
        <Marker position={position} icon={myIcon}> 
        </Marker>
</Map>


: null }

<Weather/>
      
</div>    
    );
  }
}

